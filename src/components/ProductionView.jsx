...
function ConfidenceBar({ value }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      ...
      <span style={{ fontSize: 10, fontFamily: "'DM Mono',monospace", color: "var(--t2)", width: 28 }}>
        {value}%
      </span>
    </div>
  );
...
function ProgressDots({ story }) {
  ...
  const dot = (on) => (
    <span style={{
      ...
      background: on ? "var(--t1)" : "var(--t4)",
      display: "inline-block",
    }} />
  );
  return (
    <span style={{ display: "inline-flex", gap: 3 }}>
      {dot(hasBrief)}{dot(hasVisuals)}{dot(hasAudio)}
    </span>
  );
}
...
function BriefSection({ story, brand_profile_id, onSaved }) {
  ...
  const approve = async () => {
    ...
    try {
      ...
      await recordAgentFeedback({
        agent_name:       "brief-author",
        brand_profile_id, story_id: story.id, ai_call_id: aiCallId,
  ...
  const status = !draft ? "no brief yet"
               : savedFlash ? "saved ✓"
    ...
    >
      ...
      {draft && (
        <div style={{ display: "grid", gap: 12 }}>
          ...
          <div>
            <div style={{ fontSize: 10, fontWeight: 600, color: "var(--t3)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 6 }}>
              References
            </div>
          ...
          {reasoning && (
            <div style={{ fontSize: 11, color: "var(--t3)", fontStyle: "italic", padding: "8px 12px", borderRadius: 6, background: "var(--fill)", borderLeft: "2px solid var(--gold)" }}>
              {reasoning}
            </div>
          )}
...
const textareaStyle = {
  ...inputStyle, width: "100%", minHeight: 64, resize: "vertical", lineHeight: 1.5,
};
...
function AssetMatchesSection({ story, brand_profile_id }) {
  ...
  useEffect(() => {
    setMatches(null); setGaps(null); setConf(null); setError(null);
  }, [story.id]);
  ...
  return (
    <Section
      title="Asset library matches"
      status={status}
      statusColor={matches?.length ? "success" : "default"}
      ...
      {matches != null && (
        <div style={{ display: "grid", gap: 12 }}>
          ...
          {matches.length > 0 && (
            <div>
              <div style={{ fontSize: 10, fontWeight: 600, color: "var(--t3)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 6 }}>
                Reusable matches ({matches.length})
              </div>
              <div style={{ display: "grid", gap: 6 }}>
                {matches.map((m, i) => (
                  <div key={i} style={{ fontSize: 12, padding: "8px 12px", borderRadius: 6, background: "rgba(74,155,127,0.08)", border: "0.5px solid rgba(74,155,127,0.3)" }}>
                    <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 10, fontWeight: 600, color: "#4A9B7F", marginRight: 8 }}>
                      {m.position_key}
                    </span>
          ...
          {gaps && gaps.length > 0 && (
            <div>
              <div style={{ fontSize: 10, fontWeight: 600, color: "var(--t3)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 6 }}>
                Gaps to generate ({gaps.length})
              </div>
              <div style={{ display: "grid", gap: 6 }}>
                {gaps.map((g, i) => (
                  <div key={i} style={{ fontSize: 12, padding: "8px 12px", borderRadius: 6, background: "var(--fill)", border: "0.5px solid var(--border)" }}>
                    <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 10, fontWeight: 600, color: "var(--t2)", marginRight: 8 }}>
                      {g.position_key} × {g.count}
                    </span>
          ...
          <div>
            <button onClick={run} disabled={running} style={btnSecondary}>
              <RefreshCw size={12} />
...
function StubSection({ Icon, title, description, deliveryLabel }) {
  return (
    <Section
      title={
        <span style={{ display: "inline-flex", alignItems: "center", gap: 6, color: "var(--t3)" }}>
          <Icon size={12} />
          {title}
        </span>
      }
  ...
  return (
    <div>
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 6 }}>
        ...
        <span style={{ fontSize: 12, color: "var(--t3)", fontFamily: "'DM Mono',monospace" }}>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  borderLeftColor: formatBorder(story.format),
                  cursor: "pointer",
                  boxShadow: isSelected ? "var(--shadow-sm)" : "none",
                  transition: "background 0.12s",
                }}
              >
                <div style={{ fontSize: 13, fontWeight: isSelected ? 600 : 500, color: "var(--t1)", lineHeight: 1.3, marginBottom: 4 }}>
          ...
          {selected && (
            <>
              <div style={{ paddingBottom: 14, borderBottom: "0.5px solid var(--border)", marginBottom: 4 }}>
                <div style={{ fontSize: 16, fontWeight: 600, color: "var(--t1)", lineHeight: 1.3 }}>
                  {selected.title}
                </div>
